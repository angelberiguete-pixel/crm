...
export default function LoginPage() {
  ...
  useEffect(() => { supabase.auth.getSession().then(({data}) => { if (data.session) router.replace("/revenue-command-center"); }); }, [router]);
  ...
  async function submit(e: FormEvent) {
    ...
    if (result.data.session) router.replace("/revenue-command-center");
  ...
  return <main className="auth-shell">
    <section className="auth-panel">
      ...
      <p className="muted">Gestiona prospectos, pipeline, MRR y las acciones que te acercan a RD$1,000,000 mensuales recurrentes.</p>
